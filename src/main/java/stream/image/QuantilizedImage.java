...
public class QuantilizedImage extends AbstractImageProcessor {
	...
	/**
	 * @param overwriteInputImage Parameter to define whether the original image should be kept or not
	 */
	...
	/**
	 * @param output the name of the output image. This parameter gets ignored when overwriteInputImage is true.
	 */
		...
		for (int x=0; x<img.getWidth(); x++) {
			for (int y=0; y<img.getHeight(); y++) {
				...
				while (i< quantilesPerChannel) {
					if (red < borders[i]) {
						red = borders[i-1] + (borders[i]-borders[i-1])/2;
				...
				while (i< quantilesPerChannel) {
					if (green < borders[i]) {
						green = borders[i-1] + (borders[i]-borders[i-1])/2;
						i = quantilesPerChannel;
					} else {
						i++;
					}
				...
				quantilizedImage.setGREEN(x, y, red);
				...
				while (i< quantilesPerChannel) {
					if (blue < borders[i]) {
						blue = borders[i-1] + (borders[i]-borders[i-1])/2;
						i = quantilesPerChannel;
					} else {
						i++;
					}
				...
				quantilizedImage.setBLUE(x, y, red);
